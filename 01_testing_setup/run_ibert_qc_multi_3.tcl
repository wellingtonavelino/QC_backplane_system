...
# --- 1.0) User parameters (make sure these appear before you ever use $cfg_file or $max_boards)
# ———————————————————————————————————————————————————————————————————————————————— #
...
# --- 1.1) Locate & launch hw_server.bat by searching up from vivado.exe ----------- #
...
puts "PYTHON_OUT: vivado.exe = $vivado_exe"
...
puts "PYTHON_OUT:vivado.exe =  launching hw_server → $hw_server_bat"
if {[catch {
...
# --- 1.2) Open & connect Hardware Manager -------------------------------------- #
open_hw_manager
after 800
...
# --- 1.3) Read config and collect up to max_boards entries --------------------- #
set boards {}
...
# --- 1.4) Phase 1: Program & setup links for each board ------------------------ #
set idx 0
foreach entry $boards {
	...
	puts "\n=== Board #$idx: $serial ==="
	puts "PYTHON_OUT:=== Board #$idx: $serial ==="
	incr idx

    # 1.4.1) Find the target by serial
    set tgt ""
	...
	puts "PYTHON_OUT:OK: target = [get_property NAME $tgt] (UID=$serial)"
    ...
    current_hw_target $tgt
     ...
     # 1.4.2) Open this target (now that it's current)
	...
	puts "PYTHON_OUT:OK: FPGA device = [get_property NAME $fpga_dev]"
	...
	after 3000  ;# Let the IBERT core come up fully
	puts "Done."
    ...
    for {set j 0} {$j < [llength $links]} {incr j 2} {
		...
		puts "PYTHON_OUT:Scheduled test: Board#$idx $label"
    }
